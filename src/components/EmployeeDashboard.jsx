...
You have clocked in present ${presentCount} times in the database history logs.`;
        ...
        response = `Your base salary is Rs. ${currentUser.salary.toLocaleString()}/month. A total of ${userPayslips.length} payslips have been released by HR. You can view/print statements in the "Salary Ledger" tab.`;
      } 
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card glass-panel info">
          <div className="stat-info">
            ...
            <span className="stat-value">Rs. {currentUser.salary.toLocaleString()}</span>
          </div>
          <div className="stat-icon-wrapper">
                ...
                <tbody>
                  {userPayslips.map((ps) => (
                    <tr key={ps.id}>
                      ...
                      <td><strong style={{ color: 'var(--color-success)' }}>Rs. {ps.netSalary.toLocaleString()}</strong></td>
                      <td>{ps.releasedOn}</td>
                      <td>
                        <button 
            ...
            <div className="payslip-layout">
              ...
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '0.75rem' }}>
                <div className="payslip-row">
                  ...
                  <span>Rs. {viewingPayslip.baseSalary.toLocaleString()}</span>
                </div>
                <div className="payslip-row">
                  <span style={{ color: 'var(--text-muted)' }}>Performance Bonus (+)</span>
                  <span style={{ color: 'var(--color-success)' }}>+Rs. {viewingPayslip.bonus.toLocaleString()}</span>
                </div>
                <div className="payslip-row" style={{ paddingBottom: '0.75rem' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Regulatory Deductions (-)</span>
                  <span style={{ color: 'var(--color-danger)' }}>-Rs. {viewingPayslip.deductions.toLocaleString()}</span>
                </div>
                <div className="payslip-row payslip-total">
                  <span>NET DISBURSED COMP</span>
                  <span>Rs. {viewingPayslip.netSalary.toLocaleString()}</span>
                </div>
              </div>
            </div>
